    ...
    $.extend({
        ...
        operate: {
            ...
            closeWork: function(id) {
                table.set();
                $.modal.open("添加" + table.options.modalName, $.operate.closeWorkUrl(id));
            },
            // 添加信息
            add: function(id) {
            	table.set();
            ...
            //下班打卡地址
            closeWorkUrl: function(id) {
                var url = $.common.isEmpty(id) ? table.options.closeWorkUrl.replace("{id}", "") : table.options.closeWorkUrl.replace("{id}", id);
                return url;
            },
            // 修改信息
            edit: function(id) {
            ...
            shenpi: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.shenpiUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("修改" + table.options.modalName, url);
                } else {
                    $.modal.open("修改" + table.options.modalName, $.operate.shenpiUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            //审批访问地址
            shenpiUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.shenpiUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
